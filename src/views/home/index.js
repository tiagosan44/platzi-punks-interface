...
const Home = () => {
  ...
  const getPlatziPunksData = useCallback(async () => {
    if (platziPunks) {
      var totalSupply = await platziPunks.methods.totalSupply().call();
  ...
  const mint = () => {
    ...
    platziPunks.methods
      ...
      .on("receipt", () => {
        setIsMinting(false);
        toast({
          title: 'Trnsanccion confirmada',
          description: 'Nunca apres de aprender',
          status: 'success'
        })
      })
      .on("error", (error) => {
